...
def compute_qkv(X, W_q, W_k, W_v):
    ...
    W_q, W_k, W_v: 投影矩阵 (d_model, d_k/d_k/d_v)
    """
...
def attention_scores(Q, K):
    ...
    scores[i,j] = 词 i 的 query 对 词 j 的 key 的相关程度
    """
    d_k = K.shape[-1]
...
def self_attention(X, d_k, d_v, W_q, W_k, W_v):
    ...
    # ===== Step 4: 加权求和 =====
    context = np.dot(weights, V)  # (seq_len, d_v)
...
# 计算注意力
context, weights = self_attention(X, d_k, d_v, W_q, W_k, W_v)
...
## 第六部分：多头注意力（Multi-Head Attention）
...
## 第七部分：Self-Attention vs Cross-Attention
...
# 假设这是翻译 "今天天气很好" → "Today weather is good" 的注意力
attention_matrix = np.array([
    [0.8, 0.1, 0.05, 0.05],  # "Today" 关注 "今天"
    [0.1, 0.75, 0.1, 0.05],  # "weather" 关注 "天气"
    [0.05, 0.1, 0.6, 0.25],  # "is" 关注 "很"
    [0.05, 0.05, 0.3, 0.6],  # "good" 关注 "好"
])

plt.figure(figsize=(8, 6))
plt.imshow(attention_matrix, cmap='Blues')

plt.xticks(range(4), ['今天', '天气', '很', '好'])
plt.yticks(range(4), ['Today', 'weather', 'is', 'good'])
plt.xlabel('中文（输入）')
plt.ylabel('英文（输出）')
plt.title('Attention 权重可视化')
