...
result_horizontal = conv2d(image, horizontal_kernel)
...
```python
# 多通道卷积
def conv2d_multi_channel(image, kernels, bias):
    """
    image: (H, W, C_in) - 多通道输入
    kernels: (C_out, K, K, C_in) - 多个卷积核
    """
    n_out, k, k2, c_in = kernels.shape
    h, w = image.shape[:2]
    out_h = (h - k) // stride + 1
    out_w = (w - k) // stride + 1
    output = np.zeros((out_h, out_w, n_out))

    for c in range(n_out):
        for i in range(0, h - k + 1, stride):
            for j in range(0, w - k + 1, stride):
                patch = image[i:i+k, j:j+k]
                output[i//stride, j//stride, c] = np.sum(patch * kernels[c]) + bias[c]
